...
def eval_sequence(model_trainier: ModelTrainer,
  ...
  test_loss, predictions = model_trainier.eval(data.data_dict[stock_ticker].test_x, data.data_dict[stock_ticker].test_y)

  if print_result:
    print(f'Test Loss: {test_loss:.4f}')
  # print(f"test_indices: {data.data_dict[stock_ticker].test_indices}\n\n")
  if plot_result:
    plotting.plot_sequence_prediction_comparison(data.data_dict[stock_ticker].test_data[model_trainier.output_features],
                                                 predictions,
                                                 data.data_dict[stock_ticker].test_indices[model_trainier.look_back:])
...
def eval(model_trainier: ModelTrainer,
  ...
  if plot_result:
    plotting.plot_sequence_prediction_comparison(data.data_dict[stock_ticker].test_data[model_trainier.output_features],
                                                 predictions,
                                                 data.data_dict[stock_ticker].test_indices)
  return data, test_loss, predictions
